...
// Renamed substrate.h --> Substrate.h to fit with naming pattern.
...
#ifdef __arm__
template <typename Type_>
__attribute__((__deprecated__)) static inline Type_ *MSHookMessage(Class _class, SEL sel, Type_ *imp, const char *prefix = NULL) {
...
#define MSRegister_(name, dollar, colon)                                           \
    namespace {                                                                    \
        static class C_$##name##$##dollar {                                        \
        public:                                                                    \
            _finline C_$##name##$##dollar() {                                      \
...
#define MSMessage_(extra, type, _class, name, dollar, colon, call, args...)                                                                                                         \
    ...
    extra(name, dollar, colon) static _finline type _$##name##$##dollar(Class _cls, type (*_old)(_class, SEL, ##args, ...), type (*_spr)(struct objc_super *, SEL, ##args, ...), _class self, SEL _cmd, ##args)
...
#define MSInitialize                   \
    ...
    namespace {                        \
        static class $MSInitialize {   \
        public:                        \
            _finline $MSInitialize() { \
                _MSInitialize();       \
            }                          \
        } $MSInitialize;               \
    }                                  \
