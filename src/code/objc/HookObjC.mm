...
bool hookMethodName(SEL name, Class target, Class hook) {
    ...
    if(std::strcmp(origType, hookType) != 0) {
        std::cout << "type encoding mismatch - method " << sel_getName(name) << " should return " << origType << ", but hook returns " << hookType << '\n';
        return false;
    }
    ...
    if(not class_addMethod(target, origSelector, targetImplementation, origType)) {
        std::cout << "failed to add orig method for selector " << sel_getName(name) << " to class " << class_getName(target) << '\n';
        return false;
    }
...
static auto methodComparator = [](const Method &a, const Method &b) {
    return std::hash<std::string>()(sel_getName(method_getName(a))) > std::hash<std::string>()(sel_getName(method_getName(b)));
};
...
std::set<Method, decltype(methodComparator)> getMethods(Class cls) {
    ...
    for(unsigned i = 0; i < count; ++i) methodSet.insert(methods[i]);
